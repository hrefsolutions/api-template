...
export const MINIMUM_STAY_DAYS = 2;

export const normalizeDateToStartOfDay = (value: Date | string): Date => {
  const date = value instanceof Date ? new Date(value.getTime()) : new Date(value);
  date.setHours(0, 0, 0, 0);
  return date;
};

export const normalizeDateToEndOfDay = (value: Date | string): Date => {
  const date = value instanceof Date ? new Date(value.getTime()) : new Date(value);
...
export const calculateStayLengthInDays = (startDate: Date | string, endDate: Date | string): number => {
  const normalizedStart = normalizeDateToStartOfDay(startDate);
  const normalizedEnd = normalizeDateToStartOfDay(endDate);

  if (Number.isNaN(normalizedStart.getTime()) || Number.isNaN(normalizedEnd.getTime())) {
    return Number.NaN;
  }

  return (normalizedEnd.getTime() - normalizedStart.getTime()) / MILLISECONDS_PER_DAY;
};

export const isMinimumStayMet = (
  startDate: Date | string,
  endDate: Date | string,
  minimumDays: number = MINIMUM_STAY_DAYS
): boolean => {
  const stayLength = calculateStayLengthInDays(startDate, endDate);
  return !Number.isNaN(stayLength) && stayLength >= minimumDays;
};
